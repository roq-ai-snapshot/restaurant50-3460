...
import { getTableSectionById } from 'apiSdk/table-sections';
  ...
  const { data, error, isLoading, mutate } = useSWR<TableSectionInterface>(
    ...
    () =>
      getTableSectionById(id, {
        relations: ['restaurant', 'user', 'order'],
      }),
  );
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          <Text as="h1" fontSize="2xl" fontWeight="bold">
            Table Section Detail View
          </Text>
          {hasAccess('table_section', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/table-sections/edit/${data?.id}`} passHref legacyBehavior>
              <Button
              ...
              {hasAccess('order', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/orders/create?table_section_id=${data?.id}`}>
