...
function OrderViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<OrderInterface>(
    ...
    () =>
      getOrderById(id, {
        relations: ['user_order_customer_idTouser', 'user_order_wait_staff_idTouser', 'table_section', 'order_item'],
      }),
  );
            ...
            <Box>
              ...
              {hasAccess('table_section', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Flex alignItems="center" mb={4}>
                  ...
                  <Text fontSize="md" as="span" ml={3}>
                    <Link as={NextLink} href={`/table-sections/view/${data?.table_section?.id}`}>
                      {data?.table_section?.name}
                    </Link>
                  </Text>
                </Flex>
              )}
